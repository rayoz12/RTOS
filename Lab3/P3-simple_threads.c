...
main(void)
{
  ...
  if (pthread_create(&thread2, 
	...
	* what is the result?
	* 
	* The threads won't get a chance to run becuase the main thread doesn't wait for them
	* Meaning that the program will complete before the threads start making the OS kill the threads at the same time.
	* The pthread_join function makes the calling thread waiting for the thread passed.
	*/
